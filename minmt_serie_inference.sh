...
fmod=$dnet/network.checkpoint_00415000.pt ####### A MODIF
	...
	for corpus in EMEA; do
      ...
      CUDA_VISIBLE_DEVICES=0 python3 $trans -dnet $dnet -m $fmod -batch_size 10 -beam_size 2 -alpha 0.7 -i_src $fsrc -i_sim $fsim -i_pre $fpre -o $fout -cuda -log_file $fout.log &
